...
#                      #
###                  ###
cutoff.short  <- sqrt(sqrt(2.35 - min(dataset.raw$ISI)))
cutoff.long   <- sqrt(sqrt(4 - min(dataset.raw$ISI)))
cutoff.longer <- sqrt(sqrt(6 - min(dataset.raw$ISI)))
dataset.without.short.ISI <- dataset[dataset$ISI > cutoff.short,]
dataset.without.long.ISI <- dataset[dataset$ISI < cutoff.long,]
dataset.without.extreme.ISI <- dataset[dataset$ISI > cutoff.short & dataset$ISI < cutoff.longer,]
m9.without.short.ISI <- lmer(ResponseFDI ~ 1 + Condition + ISI + PowerC3H + PowerC4H + Condition:ISI + Condition:PowerC3H + Condition:PowerC4H + ISI:PowerC3H + PowerC3H:PowerC4H + (1+Condition|Subject), 
                             data=dataset.without.short.ISI, REML=F) 
m9.without.long.ISI <- lmer(ResponseFDI ~ 1 + Condition + ISI + PowerC3H + PowerC4H + Condition:ISI + Condition:PowerC3H + Condition:PowerC4H + ISI:PowerC3H + PowerC3H:PowerC4H + (1+Condition|Subject), 
                             data=dataset.without.long.ISI, REML=F) 
m9.without.extreme.ISI <- lmer(ResponseFDI ~ 1 + Condition + ISI + PowerC3H + PowerC4H + Condition:ISI + Condition:PowerC3H + Condition:PowerC4H + ISI:PowerC3H + PowerC3H:PowerC4H + (1+Condition|Subject), 
                             data=dataset.without.extreme.ISI, REML=F) 

car::Anova(m9.without.short.ISI)
nrow(dataset.without.short.ISI)/nrow(dataset)
car::Anova(m9.without.long.ISI)
nrow(dataset.without.long.ISI)/nrow(dataset)
car::Anova(m9.without.extreme.ISI)
nrow(dataset.without.extreme.ISI)/nrow(dataset)


fixef(m9.without.short.ISI)
fixef(m9.without.long.ISI)
fixef(m9.without.extreme.ISI)
fixef(m9)


m8.without.short.ISI <- lmer(ResponseFDI ~ 1 + Condition + ISI + PowerC3H + PowerC4H + Condition:PowerC3H + Condition:PowerC4H + ISI:PowerC3H + PowerC3H:PowerC4H + (1+Condition|Subject), 
                             data=dataset.without.short.ISI, REML=F) 
m8.without.extreme.ISI <- lmer(ResponseFDI ~ 1 + Condition + ISI + PowerC3H + PowerC4H + Condition:PowerC3H + Condition:PowerC4H + ISI:PowerC3H + PowerC3H:PowerC4H + (1+Condition|Subject), 
                             data=dataset.without.extreme.ISI, REML=F) 
lrtest(m8.without.short.ISI, m9.without.short.ISI)
lrtest(m8.without.extreme.ISI, m9.without.extreme.ISI)



sjRes.m9.wo.short <- sjPlot::plot_model(m9.without.short.ISI, show.p = TRUE,show.value=TRUE, 
                               vline.color = "gray90",value.offset=0.3,
                               dot.size=1, colors="black")  + 
  labs(title=element_blank(), y="Estimated fixed effect on ResponseFDI") +
  scale_x_discrete(labels=rev(rownames(as.data.frame(car::Anova(m9.without.short.ISI))))) +
  ylim(-0.35, 0.35) + 
  theme_bw() + 
  title("Without short ISIs") +
  theme(axis.text=element_text(size=10), aspect.ratio = 1)
# correct p-values to Wald chi2 type II
p <- as.data.frame(car::Anova(m9.without.short.ISI))$`Pr(>Chisq)`
star.labels <- ifelse(p < 0.001, "***", ifelse(p < 0.01, "**", ifelse(p < 0.05, "*", "")))
fe <- round(fixef(m9.without.short.ISI), digits=3)
sjRes.m9.wo.short$data$p.stars <- star.labels
sjRes.m9.wo.short$data$p.value <- p
sjRes.m9.wo.short$data$p.label <- sprintf("%g %s", fe[2:length(fe)], star.labels)
sjRes.m9.wo.short

sjRes.m9.wo.long <- sjPlot::plot_model(m9.without.long.ISI, show.p = TRUE,show.value=TRUE, 
                                        vline.color = "gray90",value.offset=0.3,
                                        dot.size=1, colors="black")  + 
  labs(title=element_blank(), y="Estimated fixed effect on ResponseFDI") +
  scale_x_discrete(labels=rev(rownames(as.data.frame(car::Anova(m9.without.long.ISI))))) +
  ylim(-0.35, 0.35) + 
  theme_bw() + 
  title("Without long ISIs") +
  theme(axis.text=element_text(size=10), aspect.ratio = 1)
# correct p-values to Wald chi2 type II
p <- as.data.frame(car::Anova(m9.without.long.ISI))$`Pr(>Chisq)`
star.labels <- ifelse(p < 0.001, "***", ifelse(p < 0.01, "**", ifelse(p < 0.05, "*", "")))
fe <- round(fixef(m9.without.long.ISI), digits=3)
sjRes.m9.wo.long$data$p.stars <- star.labels
sjRes.m9.wo.long$data$p.value <- p
sjRes.m9.wo.long$data$p.label <- sprintf("%g %s", fe[2:length(fe)], star.labels)
sjRes.m9.wo.long

sjRes.m9.wo.extreme <- sjPlot::plot_model(m9.without.extreme.ISI, show.p = TRUE,show.value=TRUE, 
                                        vline.color = "gray90",value.offset=0.3,
                                        dot.size=1, colors="black")  + 
  labs(title=element_blank(), y="Estimated fixed effect on ResponseFDI") +
  scale_x_discrete(labels=rev(rownames(as.data.frame(car::Anova(m9.without.extreme.ISI))))) +
  ylim(-0.35, 0.35) + 
  theme_bw() + 
  title("Without extreme ISIs") +
  theme(axis.text=element_text(size=10), aspect.ratio = 1)
# correct p-values to Wald chi2 type II
p <- as.data.frame(car::Anova(m9.without.extreme.ISI))$`Pr(>Chisq)`
star.labels <- ifelse(p < 0.001, "***", ifelse(p < 0.01, "**", ifelse(p < 0.05, "*", "")))
fe <- round(fixef(m9.without.extreme.ISI), digits=3)
sjRes.m9.wo.extreme$data$p.stars <- star.labels
sjRes.m9.wo.extreme$data$p.value <- p
sjRes.m9.wo.extreme$data$p.label <- sprintf("%g %s", fe[2:length(fe)], star.labels)
sjRes.m9.wo.extreme


ggsave(
  plot = sjRes.m9.wo.long | sjRes.m9.wo.short | sjRes.m9.wo.extreme,
  filename = "cropped_ISI.pdf",
  bg = "transparent", width=4500, height=1500,units="px"
)
